...
from . import prompt_hook, readers
...
class AttachBridge:
    ...
    def _newest_under(base: Path, *patterns: str) -> Path | None:
        ...
        for pat in (patterns or ("*.jsonl",)):
            ...
            # In-process subagents write their own transcripts under <session>/subagents/ —
            # never the log of the session we drive, so they must not win the "newest" race.
